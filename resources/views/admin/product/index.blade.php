...
<div class="row">
    <div class="col-12">
        <button class="btn-primary btn" onclick="tambah_bahan()">Tambah Product</button>
        <button class="btn btn-danger d-none" onclick="add_excel()">Import Excel</button>
    </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form class="form-horizontal " onsubmit="return addbahan();">
                ...
                <div class="form-group">
                    <label  class="col-6">Lama Process (hitungan hari)</label>
                    <div class="col-8">
                        <input type="number" id="add-lama-proses" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    <label  class="col-2">Harga</label>
                    <div class="col-8">
                        <input type="number" id="add-harga" class="form-control">
                    </div>
                </div>
            </div>
        <div class="modal-footer">
...
<script>
    ...
    function list_cat(){
        $.ajax({
            method:"get",
            url:`${api}/v1/export/excel/product-category`,
            headers:{
    ...
    $.ajax({
        method:"get",
        url:api+'/v1/product?with=category,satuan',
        headers:{
            Authorization:`Bearer ${localStorage.getItem('token')}`
        },
        success:res=>{
    ...
    function filter(){
        ...
        let uri=api+'/v1/product?with=category,satuan'
        let filters=[];
        if (cat!='') {
    ...
    function template(){
        $.ajax({
            method:"get",
            url:`${api}/v1/export/excel/product`,
            headers:{
                Authorization:`Bearer ${localStorage.getItem('token')}`
            },
            responseType: 'arraybuffer', // Set the response type to 'arraybuffer'
    ...
    function addbahan(){
       let data={}
        ...
        data.category_product_id=$("#add-category").val()
        data.satuan_product_id=$("#add-satuan").val()
        data.harga=$("#add-harga").val()
        data.time_product=$('#add-lama-proses').val()
        data.stok=0
       $.ajax({
        method:"POST",
        url:api+'/v1/product',
    ...
    function edit(id){
        window.location.replace(url+`/product/${id}`)
    }
